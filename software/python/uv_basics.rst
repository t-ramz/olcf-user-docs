...
This guide introduces a user to the basic workflow of using uv virtual environments, as well as providing an example of how to create a conda environment that uses a different version of Python than the base environment.
...
Next you will need to install ``uv``, which we will use from here onward to issue commands.
You have multiple options to choose from

.. tab-set:: Install Guide
    ...
    .. tab-item:: Github Releases
        ...
        .. code-block:: bash
            ...
            # curl -LsSO
            wget https://github.com/astral-sh/uv/releases/download/0.9.28/uv-x86_64-unknown-linux-gnu.tar.gz
            tar -xf uv-x86_64-unknown-linux-gnu.tar.gz

            # TODO: Potentially dangerous. Consider moving to .local/bin or similar
            export PATH=${PWD}/uv-x86_64-unknown-linux-gnu:${PATH}



``uv`` uses system environments when possible, so you should still see the local Python version initially.
To see a list of Python versions visible to uv, use ``uv python list``:

.. code-block:: bash

   uv python list

   ... some list of pythons and their paths/locations

It is easy to then create and use a new virtual environment for uv with syntax similar to the ``venv`` module of Python.
If the version of Python you specify is not already available as a system package, uv will install it for you in your home directory.

.. code-block:: bash

    uv venv --python=3.12 --no-cache-dir ./py-312

    source ./py-312/bin/activate

To see what packages are installed in the active environment, use ``uv pip list``:
...
.. code-block:: bash
   ...
   Using Python 3.14.2 environment at: torch-env
   Package           Version
   ----------------- ------------------
   filelock          3.20.3
   fsspec            2026.1.0
   ...
   setuptools        78.1.0
   sympy             1.14.0
   torch             2.11.0.dev20260201
   torchvision       0.25.0.dev20260201
...
Installing packages is quick and easy with the ``uv pip`` interface
...
TODO:
* the docs need to be re-ordered. It would be helpful to display ``uv`` in conjunction with Conda/Miniforge3
* Briefly overview benefits of ``uv``
    speed
    caching and re-use
* overview of project feature for more complete coverage
    specifically package spec for torch projects with uv add index-urls
===========================================================================================
